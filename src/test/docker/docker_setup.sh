...
if [ $# -eq 0 ]; then
  ...
  if [ -z "$POSTGRES_IMAGE_PATH" ]; then
    export POSTGRES_IMAGE_PATH=armdocker.rnd.ericsson.se/dockerhub-ericsson-remote
  fi
  if [ -z "$GERRIT_IMAGE_PATH" ]; then
    export GERRIT_IMAGE_PATH=armdocker.rnd.ericsson.se/dockerhub-ericsson-remote/gerritcodereview
  fi
