...
if [ $# -eq 0 ]; then
  ...
  while [ $# -ne 0 ]; do
    ...
    "--help")
      ...
      echo
      echo "[--httpd-image-path]           Apache httpd image path without image name (optional)"
      echo
  ...
  if [ -z "$HTTPD_IMAGE_PATH" ]; then
    export HTTPD_IMAGE_PATH=httpd
  fi
  ...
  docker-compose build --build-arg POSTGRES_DRIVER=$POSTGRES_DRIVER_PATH \
    ...
    --build-arg HTTPD_IMAGE=$HTTPD_IMAGE_PATH
